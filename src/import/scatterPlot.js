...
const scatterPlot = function chart() {
  ...
  const buildScales = (cs) => {
    cs.y.scale = d3.scaleLinear()
      .domain([this.minTriplet.v2 - this.maxTriplet.v2 * 0.05, this.maxTriplet.v2 + this.maxTriplet.v2 * 0.05])
      .range([this.displayHeight - cs.x.axisHeight, this.header]);
    cs.x.scale = d3.scaleLinear()
      .domain([this.minTriplet.v1 - this.maxTriplet.v2 * 0.05, this.maxTriplet.v1 + this.maxTriplet.v1 * 0.05])
      .range([0, this.width]);
  };
  /**
